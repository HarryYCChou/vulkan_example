...
DePipeline::~DePipeline() {
  vkDestroyPipeline(de_device.device(), graphics_pipeline, nullptr);
}
...
void DePipeline::CreateGraphicsPipeline(
  ...
  auto bindingDescriptions = DeModel::Vertex::GetBindingDescriptions();
  auto attributeDescriptions = DeModel::Vertex::GetAttributeDescriptions();
  ...
  pipelineInfo.pDynamicState = &config_info.dynamicStateInfo;  // Optional
  ...
  vkDestroyShaderModule(de_device.device(), vert_shader_module, nullptr);
  vkDestroyShaderModule(de_device.device(), frag_shader_module, nullptr);
}
...
void DePipeline::DefaultPipelineConfigInfo(PipelineConfigInfo& configInfo) {
